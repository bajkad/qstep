---
title: 'Preprocessing text and analyzing word frequencies'
...
For this tutorial we will mainly be relying on basic preprocessing. At the end of the tutorial we include a section on using advanced preprocessing.
...
This time we're passing a data.frame to `create_tcorpus`. We therefore need to specify which column contains the `text`. (If there is a unique ID in the data, you can use the doc_column argument to use it. Otherwise, corpustools with simply use the row indices as IDs).
